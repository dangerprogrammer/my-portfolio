import { pageContent, imageContainer, notClickStyles, preLoaders, contentImage, mainTitle } from './PageContent.module.scss';

function PageContent({ titlePage, notClicking: notClick, children, ...contexts }) {
    return <section {...contexts} style={{display: 'flex'}}>
        <main className={pageContent} style={{zIndex: 50}}>
            <h1 className={mainTitle}>{titlePage}</h1>
            <aside className={[imageContainer, notClick ? notClickStyles : undefined].join(' ')} onClick={() => console.log("opa!")}>
