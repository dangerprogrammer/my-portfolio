function activePage(page) {
    const aTarget = page.parentElement, aSiblings = [...aTarget.parentElement.children],
        ...
        pageContainer = document.querySelector('#page-container'), scrollDown = document.querySelector('#scroll-down'), locationHash = window.location.hash;

    aSiblings.forEach(sibling => sibling.classList[sibling.href === aTarget.href ? 'add' : 'remove']('active'));

    if (sectionTop > 0 || locationHash) page.click();
...
function activeNextPage() {
    ...
    let nextPage;
    siblings.forEach(({href}, ind) => {
        if (href === ownerA.href) nextPage = siblings[Math.min(ind + 1, siblings.length - 1)].firstChild;
    });
...
function activePrevPage() {
    ...
    let prevPage;
    siblings.forEach(({href}, ind) => {
        if (href === ownerA.href) prevPage = siblings[Math.max(ind - 1, 0)].firstChild;
    });
